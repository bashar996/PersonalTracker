...
import { dueInfo } from '../lib/helpers';

export default function TaskRow({ task, project, onOpen, onToggleStatus }) {
  const done = task.status === 'done';
  const di = dueInfo(task);
  const showPrio = task.priority === 'high' || task.priority === 'med';
  const prioColor = task.priority === 'high' ? '#c0564a' : '#c99a3f';
      ...
      <div className="task-main">
        <div className="task-top-row">
          ...
          <div className="task-meta-icons">
            ...
            {showPrio && <span className="priority-dot" style={{ background: prioColor }} />}
          </div>
        </div>
        <div className="task-bottom-row">
          ...
          {di && <span className={dueCls}>{di.label}</span>}
          {task.type === 'meeting' && <span className="badge-meeting">Meeting</span>}
          {tags.length > 0 && (
